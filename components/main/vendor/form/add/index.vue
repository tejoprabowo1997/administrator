...
const picName = ref<string | null>(null)
const picPhone = ref<string | null>(null)
const picEmail = ref<string | null>(null)
...
const submit = async () => {
  ...
  if (!categories.value) return helperStore.setNotify(true, 'Validasi Error', 'Kategori harus diisi', 'error')
  ...
  if (!picture.value || picture.value.length === 0)
    return helperStore.setNotify(true, 'Validasi Error', 'Photo Vendor harus diisi', 'error')
  ...
  const imgToBase64 = await useToBase64(picture.value)
  ...
  if (responseApi.value.success) formAddVendor.value = false
}
</script>
...
<template>
  <div>
    <q-form @submit="submit">
      <q-card>
        <q-card-section>
          <div class="text-h6 text-grey-7">Tambah data vendor baru</div>
        </q-card-section>
        ...
        <q-card-section horizontal>
          ...
          <q-card-section class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
            <q-select
              ...
              :rules="[(val) => (val && val.length > 0) || 'Pilih setidaknya satu kategori']"
            >
        ...
        <q-card-section horizontal>
          ...
          <q-card-section class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
            <q-file
              ...
              clearable
              :rules="[
                (val: File) => !!val || 'Foto harus diisi',
                (val: File) =>
                  !!['image/jpeg', 'image/png', 'image/gif'].includes(val.type) ||
                  'Format file harus berupa gambar (JPEG, PNG, GIF)',
                (val: File) => !!(val.size <= 2 * 1024 * 1024) || 'Ukuran file tidak boleh lebih dari 2MB',
              ]"
            />
          </q-card-section>
        </q-card-section>
        ...
        <q-card-actions align="right">
          <q-btn
            ...
            "
          />
          <q-btn type="submit" label="Simpan" flat color="blue-6" :loading="loading" />
        </q-card-actions>
      </q-card>
    </q-form>
  </div>
